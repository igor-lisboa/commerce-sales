...
<form method="POST" action="<?= route(($product ?? null ? 'product.update' : 'product.store'), ($product ?? null ? [$product] : [])) ?>">
    <fieldset>
        ...
        <input placeholder="Preço" type="number" min="0" step="0.01" name="price_cents" required value="<?= old('price_cents') ?? ($product->price ?? '') ?>" />
        <input placeholder="Promoção Preço" type="number" min="0" step="0.01" name="price_cents_promotion" value="<?= old('price_cents_promotion') ?? ($product->price_promotion ?? '') ?>" />
